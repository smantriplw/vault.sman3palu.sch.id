...
export function SecretsPage() {
  ...
  const categories = [
    "all",
    ...new Set(secrets.map((s) => s.category).filter(Boolean)),
  ];
  ...
  const toggleReveal = useCallback(
    async (id: string) => {
      ...
      setLoadingId(id);
      try {
        const { data } = await api.revealSecret(id);
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <Link to="/secrets/import" className="btn-secondary text-sm">
              Import
            </Link>
            <Link to="/secrets/export" className="btn-secondary text-sm">
              Export
            </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((secret) => {
            ...
            const data = revealedData[secret.id];
            const isLoadingSecret = loadingId === secret.id;
            return (
              <div
                key={secret.id}
                ...
                <div className="card-body">
                  ...
                  <div className="space-y-1.5 mb-3">
                    {(
                      ...
                      >
                        <span className="text-xs font-medium text-gray-500 w-24 truncate">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {isRevealed && data && (
                            <button
                              onClick={() => copyValue(data[field.key] ?? "")}
                    ...
                    {!isRevealed && !isLoadingSecret && (
                      <button
                        onClick={() => toggleReveal(secret.id)}
                        className="w-full py-2 flex items-center justify-center gap-2 text-sm text-gray-400 hover:text-brand-600 border-2 border-dashed border-gray-200 rounded-xl hover:border-brand-300 transition-colors"
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
